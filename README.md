...
- `run` (required): the command to execute. Inline `"..."` or fenced triple-quote block. All commands are passed to `bash -euo pipefail -c`, so shell features (pipes, redirects, `&&`, variable expansion) work. The strict flags mean unset variable references and pipeline failures are treated as errors. `bash` must be on `PATH`.
